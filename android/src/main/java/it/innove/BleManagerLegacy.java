...
class BleManagerLegacy extends ReactContextBaseJavaModule implements ActivityEventListener {
	...
	public Map<String, PeripheralLegacy> peripherals = new LinkedHashMap<>();
	// scan session id


	public BleManagerLegacy(ReactApplicationContext reactContext) {
		super(reactContext);
	...
	public void scan(ReadableArray serviceUUIDs, final int scanSeconds, boolean allowDuplicates, ReadableMap options, Callback callback) {
		...
		for (Iterator<Map.Entry<String, PeripheralLegacy>> iterator = peripherals.entrySet().iterator(); iterator.hasNext(); ) {
			Map.Entry<String, PeripheralLegacy> entry = iterator.next();
	...
	public void connect(String peripheralUUID, Callback callback) {
		...
		PeripheralLegacy peripheral = peripherals.get(peripheralUUID);
		if (peripheral == null) {
			...
			if (BluetoothAdapter.checkBluetoothAddress(peripheralUUID)) {
				...
				peripheral = new PeripheralLegacy(device, reactContext);
	...
	public void startNotification(String deviceUUID, String serviceUUID, String characteristicUUID, Callback callback) {
		...
		PeripheralLegacy peripheral = peripherals.get(deviceUUID);
		if (peripheral != null){
					...
					runOnUiThread(new Runnable() {
						...
						public void run() {
							...
							if (!peripherals.containsKey(address)) {
								PeripheralLegacy peripheral = new PeripheralLegacy(device, rssi, scanRecord, reactContext);
								peripherals.put(device.getAddress(), peripheral);
								WritableMap map = peripheral.asWritableMap();
								sendEvent("BleManagerDiscoverPeripheral", map);
							} else {
								// this isn't necessary
								PeripheralLegacy peripheral = peripherals.get(address);
	...
	public void getDiscoveredPeripherals(Callback callback) {
		...
		Map <String, PeripheralLegacy> peripheralsCopy = new LinkedHashMap<>(peripherals);
		for (Map.Entry<String, PeripheralLegacy> entry : peripheralsCopy.entrySet()) {
			PeripheralLegacy peripheral = entry.getValue();
